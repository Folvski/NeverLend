...
struct Layer//слои
{
    int opacity = 255;//непрозрачность слоя (инициализировано значением по умолчанию)
...
private:
    int width = 0, height = 0, tileWidth = 0, tileHeight = 0;//инициализированы нулями
    int firstTileID = 0;//инициализировано нулем
...
Object Level::GetObject(std::string name)
{
    ...
    for (int i = 0; i < objects.size(); i++)
        if (objects[i].name == name)
            return objects[i];
    return Object(); // возвращаем пустой объект, если не нашли
}
